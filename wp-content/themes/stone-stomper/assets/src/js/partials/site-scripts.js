...
jQuery( function() {
	...
	const handleBarQuestionImageDisplay = () => {
		...
		const $gridDiv = jQuery( '.grid[data-q1-image]' );

		if ( ! $container.length || ! $gridDiv.length ) {
			return;
		}

		// Handler for question 1 (shank_41)
		jQuery( '#bar_q_shank_41' ).on( 'change', function() {
			const imageUrl = $gridDiv.data( 'q1-image' );
			if ( jQuery( this ).val() && imageUrl ) {
				$container.html( '<img src="' + imageUrl + '" alt="Question 1" class="bar-q-image" />' );
			}
		} );

		// Handler for question 2 (do35_do45)
		jQuery( '#bar_q_do35_do45' ).on( 'change', function() {
			const imageUrl = $gridDiv.data( 'q2-image' );
			if ( jQuery( this ).val() && imageUrl ) {
				$container.html( '<img src="' + imageUrl + '" alt="Question 2" class="bar-q-image" />' );
			}
		} );

		// Handler for question 3 (adjustable_hitch)
		jQuery( '#bar_q_adjustable_hitch' ).on( 'change', function() {
			const imageUrl = $gridDiv.data( 'q3-image' );
			if ( jQuery( this ).val() && imageUrl ) {
				$container.html( '<img src="' + imageUrl + '" alt="Question 3" class="bar-q-image" />' );
			}
		} );

		// Handler for question 4 (tongue_85)
		jQuery( '#bar_q_tongue_85' ).on( 'change', function() {
			const imageUrl = $gridDiv.data( 'q4-image' );
			if ( jQuery( this ).val() && imageUrl ) {
				$container.html( '<img src="' + imageUrl + '" alt="Question 4" class="bar-q-image" />' );
			}
		} );

		// Show first question image on page load
		const firstImageUrl = $gridDiv.data( 'q1-image' );
		if ( firstImageUrl ) {
			$container.html( '<img src="' + firstImageUrl + '" alt="Question 1" class="bar-q-image" />' );
		}
	};
