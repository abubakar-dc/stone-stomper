...
jQuery(function () {
  ...
  function collect() {
    const data = {};
    const excludedFields = ['original_barwidth_mm', 'original_caravan_width_mm', 'original_a_frame_length_mm'];
...
function setupImageUpload(inputId, listId, slot) {
  ...
  input.multiple = true;
  let entries = readIds(slot).slice(0, maxFiles).map(url => ({
    url
  }));
  ...
  input.addEventListener('change', function () {
    ...
    entries = entries.concat(filesToAdd.map(file => ({
      file
    })));
    input.value = '';
    showEntries(entries, list);
